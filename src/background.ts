...
async function notifyUser(titleKey: string, messageKey: string, params: Record<string, any> = {}) {
  const state = await concentrationService.getState();
  const lang = state.config.language || 'es';
  const title = getTranslation(lang, titleKey as any, params);
...
async function checkAndBlock(tabId: number, url: string) {
  const pomState = await pomodoroService.getState();
