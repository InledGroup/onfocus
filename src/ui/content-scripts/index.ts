import { SiteConfig } from '../../features/concentration/domain/types';
...
function showCountdownOverlay(config: SiteConfig, lang: Language) {
  if (document.getElementById('onfocus-overlay')) return;
  ...
  const interval = setInterval(() => {
    ...
    if (textEl) textEl.innerText = `${timeLeft}`;
    
    // Calcular el desfase del círculo
    const offset = 565.48 * (1 - timeLeft / totalTime);
    if (circleProgress) circleProgress.setAttribute('stroke-dashoffset', offset.toString());
